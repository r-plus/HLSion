...
final internal class SessionManager: NSObject, AVAssetDownloadDelegate {
    ...
    func downloadStream(_ hlsion: HLSion) {
        ...
        guard let task = session.makeAssetDownloadTask(asset: hlsion.urlAsset, assetTitle: hlsion.name, assetArtworkData: nil, options: [AVAssetDownloadTaskMinimumRequiredMediaBitrateKey: 265000]) else { return }
